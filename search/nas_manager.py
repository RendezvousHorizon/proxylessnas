...
class ArchSearchRunManager:
    ...
    def __init__(self, path, super_net, run_config: RunConfig, arch_search_config: ArchSearchConfig, local_rank: int):
        # init weight parameters & build weight_optimizer
        self.run_manager = RunManager(path, super_net, run_config, True, local_rank=local_rank)
    ...
    def load_model(self, model_fname=None):
        ...
        if torch.cuda.is_available():
            checkpoint = torch.load(model_fname, map_location=self.run_manager.device)
        else:
    ...
    def warm_up(self, warmup_epochs=25):
        ...
        for epoch in range(self.warmup_epoch, warmup_epochs):
            ...
            for i, (images, labels) in enumerate(data_loader):
                ...
                output = self.run_manager.net(images)  # forward (DistributedDataParallel)
                # loss
